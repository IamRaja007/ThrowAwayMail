...
interface ApiService {
    ...
    fun getMailMessages(@Query("login")username:String, @Query("domain")domain:String): LiveData<GenericApiResponse<List<MailBoxModel>>>  //every request to network with retrofit will give us live data

    @GET("?action=readMessage")
    fun getShowSelectedMessage(@Query("login")username:String, @Query("domain")domain:String, @Query("id")id:Int):LiveData<GenericApiResponse<ShowEmailModel>>
